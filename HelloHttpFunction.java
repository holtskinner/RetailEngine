...
public class HelloHttpFunction implements HttpFunction {
    ...
    public void service(HttpRequest request, HttpResponse response) throws Exception {
        ...
        WITH FilteredApparels AS(
                ...
                '" + searchText + "'
                AS user_text FROM apparels ORDER BY embedding <=> embedding('text-embedding-004', '" + searchText + "')::vector LIMIT 5
            ),
            PredictedLiterature AS(
                SELECT id,
                category,
                sub_category,
                uri,
                ...
                (
                    json_array_elements(
                        google_ml.predict_row(
                            model_id => 'gemini-1.5-pro:streamGenerateContent',
                            request_body => CONCAT(
                                '{"contents": [ { "role": "user", "parts": [ { "text": "Read this user search text: ',
                                user_text,
                                ' Compare it against the product inventory data set: ',
                                content,
                                ' Return a response with 3 values: 1) MATCH: if the 2 contexts are at least 85% matching or not: YES or NO 2) PERCENTAGE: percentage of match, make sure that this percentage is accurate 3) DIFFERENCE: A clear short easy description of the difference between the 2 products. Remember if the user search text says that some attribute should not be there, and the record has it, it should be a NO match." } ] } ] }'
                            )::json
                        )
                    ) -> 'candidates' -> 0 -> 'content' -> 'parts' -> 0 -> 'text'
                ) AS literature FROM FilteredApparels
            )
        SELECT
        id,
        category,
        ...
        user_text,
        literature
        FROM PredictedLiterature
        WHERE CAST(literature AS VARCHAR(500)) LIKE '%MATCH:%YES%';
        """;
    ...
    public HikariDataSource AlloyDbJdbcConnector() {
        ...
        String ALLOYDB_INSTANCE_NAME = String.format("projects/%s/locations/us-central1/clusters/shopping-cluster/instances/shopping-instance", PROJECT_ID);
